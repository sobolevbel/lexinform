...
- for deputies', President's, Senate, committee and citizens' bills may order **public
  ...
  `opiniowanie.sejm.gov.pl/RPW-29075-2026`). Submitting it means signing in — the survey
  redirects to `logowanie.sejm.gov.pl/login.jsp`, and Profil Zaufany is one of the ways in, so
  the messages do not remark on it. Once the window shuts the project page shows "Liczba ankiet".
  Government bills skip all of this: they were consulted on RCL.
...
| Sejm consultation of an RPW bill (non-government) | anyone (sign-in, e.g. Profil Zaufany) | survey (ankieta) at opiniowanie.sejm.gov.pl, linked from the project page, 30 days | consultation line + link, reminder 3 days before, notice when opinions are published |
