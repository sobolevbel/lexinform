...
| 50 | P1 | fixed | `sqlite_repo.py::list_tracked` | The wait marker now bypasses the tracking-age filter as well as the discovery watermark, so an already queued observation can finish after the grace period. Linked and discontinued bills remain excluded. | `test_waiting_supplement_survives_the_tracking_age_cutoff` |
...
| 32 | P1 | batch collection | A late answer undid an operator skip and published a card | `test_collect_respects_an_operator_skip` | Consume only while the bill still awaits that batch; superseded answers do not change its status |
| 33 | P3 | analysis queue | Positive triage was charged again after batch collection | `test_collect_does_not_pay_for_triage_again` | Persist the triage memo when enqueuing |
| 36 | P3 | dry run | Batch mode produced no full analysis or preview | `test_dry_run_still_previews_a_new_analysis_without_submitting_a_batch` | Run analysis synchronously inside the rollback transaction |
| 37 | P1 | batch repository and collection | A failed batch with unconsumed items disappeared after an interrupted write | `test_failed_batch_remains_collectible_after_interrupted_item_consumption` | Keep unfinished batches visible; register and consume their items atomically |
| 39 | P1 | OpenAI batch adapter | Error-file items and successful results in expired/cancelled batches were discarded | `test_openai_returns_failed_items_from_the_error_file`, `test_openai_terminal_batch_with_results_is_collectible` | Read both files and collect terminal partial results; reconcile missing items |
| 40 | P1 | batch result adapters | One invalid structured answer aborted later items | `test_openai_malformed_analysis_does_not_hide_the_next_item`, `test_anthropic_truncated_analysis_does_not_hide_the_next_item` | Return an item error on schema validation failure |
| 43 | P1 | collect CLI | Collection bypassed configured publication threshold and caps | `test_collect_cli_respects_configured_min_score` | Pass the settings into `RunOptions` |
| 34 | P3 | batch cost guard | Queued work bypassed the run budget | `test_batch_submission_obeys_the_run_budget` | Reserve a conservative estimate at the selected batch model's price before queuing; stop the phase at the limit |
| 35 | P1 | batch reanalysis and tracker | A completed answer could wait for the next source change despite already being ready | `test_collected_reanalysis_is_applied_even_after_discovery_watermark_moves` | Persist `reanalysis_ready` and include it in tracking regardless of discovery watermark |
| 38 | P1 | provider routing | A provider switch polled old IDs through the new backend | `test_switching_provider_does_not_poll_old_ids_on_the_new_provider` | Resolve each open batch by its saved provider; wait when that backend is unavailable |
| 41 | P3 | batch accounting | Collected tokens were absent from totals, billed at sync price, and labelled with the collecting model/prompt | `test_collected_tokens_are_in_the_run_totals`, `test_batch_result_keeps_the_model_that_answered` | Build report totals from itemized calls; mark batch usage for half-price accounting and retain submit-time prompt version |
| 42 | P1 | term rollover | Government bills moved while their batch item still named the previous term | `test_government_batch_survives_a_term_rollover` | Defer moving a government row until its submitted analysis has been applied under the original memo key |
| 30 | P1 | batch submission | A submission failure stranded bills in `batch_pending` with no durable job | `test_submit_failure_keeps_work_retryable`, `test_uncertain_submission_waits_for_explicit_reconciliation` | Persist requests before the remote call; automatically retry definitive rejections; keep ambiguous acceptance visible for provider reconciliation without duplicate submission |
