...
| 7 | P0 | candidate | `models/phases.py:369` | `_phase_after` falls through to `return None` for `Referral`, `Voting`, `CommitteeReport` and a non-trailing `End`; `next_phase is None` means "road over", so the bill gets no card and a followed one freezes. `ConstitutionalTribunalRuling` was the same shape and was closed by `abecc6a` with `tribunal_after_ruling` — these four are the rest of it | 2026-09-16 |
