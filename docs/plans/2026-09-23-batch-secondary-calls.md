...
23.09.2026. Реализация завершена; production-включение amendments ждёт гейта 0.3.

24.09.2026: последующий [архитектурный аудит](../reviews/2026-09-24-secondary-batch-audit.md)
выявил шесть проблем B45–B50. B50 затем исправлен; [B45–B49 остаются открыты](../BUGS.md#open).
До их исправления формулировку «реализация завершена»
следует читать как завершение исходных фаз, а не подтверждение готовности всех граничных сценариев.
...
Дополнительная проверка перед деплоем 24.09: [полный отчёт](../reviews/2026-09-24-batch-predeploy.md)
с dry run на production-копии, реальными Batch-запросами обоим провайдерам и тестами сбоев.

- `ea462f5`: фаза 0 — отчёты без поправок и мемо сканов.
- `759941a`: фаза 1 — типизированные вопросы/ответы, оба адаптера, legacy JSON.
- `c3aefb2`: фаза 2 — ожидание, настройки, миграция v32.
- `9e47b04`: фаза 3 — joint, без строки публикации во время ожидания.
- `559bc65`: фаза 4 — supplement, отложенное наблюдение, watermark и `/status`.
- `39c9a69`: фаза 5 — amendments реализованы, в workflow не включены.
- Финальная сверка: queued intent после успешного синхронного fallback удаляется, чтобы не
  оплатить заведомо лишний запрос. Submitting/open сохраняются для сверки и учёта позднего ответа.
- Маркер ожидания ставится уже при первом Waiting, до чтения следующего документа: последующий
  outage не теряет билл за водяным знаком.
- Свежий state-дамп восстановлен в одноразовую БД `/tmp`: v32, билл 2699 читается; batch-строк
  для проверки реальных legacy payload нет, совместимость проверена offline-тестами.
- Реальные LLM-запросы и публикации не выполнялись. Production-поллер не проверялся на VPS:
  количественный гейт уже не выполнен (0 завершённых батчей). Недельный замер остаётся задачей
  выкатки; подготовка кода от него не зависит.
- Упомянутый корпусный scratch-скрипт не входит в этот репозиторий; его перенос и повторный
  замер корпуса не выполнялись. Приведённые оценки отмечены как исторические.
