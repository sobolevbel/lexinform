...
after every run (04:23, 10:23, 16:23 UTC). To reproduce a run on it:
...
| Change what happens at the end of a Sejm term | `services/tracking/rollover.py` (what lapses, what moves), `sqlite_repo.py::list_unfinished_published` / `discontinue_unfinished` / `move_rcl_projects`, the wording in `i18n.py` (`process_discontinued`, `process_carried_over`), `test_term_rollover.py`. The term itself comes from `services/terms.py`. |
...
- `.github/workflows/daily.yml` runs the bot three times a day and pushes the dump to `state`.
