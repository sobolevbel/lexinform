"""fan_out: parallel steps, outcomes consumed in input order."""
...
def test_outcomes_keep_the_input_order_and_carry_per_item_errors() -> None:
    def step(n: int) -> int:
...
def test_result_returns_the_value_or_re_raises_the_error() -> None:
    assert Outcome(item=1, value=2).result() == 2
    with pytest.raises(ValueError, match="boom"):
        Outcome(item=1, error=ValueError("boom")).result()
...
def test_single_worker_runs_in_the_calling_thread() -> None:
    ...
    values = [o.value for o in fan_out([1, 2, 3], step, workers=1)]
...
def test_several_workers_really_run_at_once() -> None:
    barrier = threading.Barrier(3, timeout=2)  # raises unless three steps run concurrently
