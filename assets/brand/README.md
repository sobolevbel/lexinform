...
One form on every surface; only the three colours rotate, so the channel, the chat and the
bot are told apart at avatar size without reading anything.

| Colour | Hex | Where |
| --- | --- | --- |
| Ink | `#10161F` | channel ground, the glyph on paper |
| Paper | `#F4EFE6` | chat ground, the glyph on ink and on red |
| Red | `#C0362C` | the change-bar, bot ground |

## Files

`channel.svg`, `chat.svg`, `bot.svg`, `favicon.svg` are the sources; everything under `png/`
is generated from them. Telegram crops an avatar to a circle, so nothing reaches outside the
inscribed circle — the square PNGs upload as they are.
