...
- Runs once a day in GitHub Actions; state is a SQLite dump in the `state` branch. Zero infra.
...
`.github/workflows/daily.yml` runs the bot every day at 05:00 UTC and keeps its state in the
`state` branch as a plain-text SQL dump.
